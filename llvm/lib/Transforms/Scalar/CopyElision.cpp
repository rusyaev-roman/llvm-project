...
bool isApplyingLimitReached() {
  if (ClCopyElisionLimit > -1) {
    if (CopyElisionCnt >= ClCopyElisionLimit) {
      ...
      LLVM_DEBUG(dbgs() << ">> Applying limit (" << CopyElisionCnt
                        << ") is reached\n");
      return true;
    }
    ++CopyElisionCnt;
...
bool isCopyStartOrEnd(const Value &V) {
  return isCopyStart(V) || isCopyEnd(V);
}
...
public:
  ...
  bool runOnFunction(Function &F) override {
    ...
    bool IsApplied = performUltimateCopyElision(F, CV.getCopies());
...
private:
  ...
  /// do main work
  bool performUltimateCopyElision(Function &F, CopyList &Copies);
...
struct CapturesBeforeCopy : public CaptureTracker {
  ...
  explicit CapturesBeforeCopy(const CopyElisionLegacyPass &CEP)
...
private:
  const CopyElisionLegacyPass &CEP;
};
...
bool CopyElisionLegacyPass::analyzeCopyAndCollectInfo(
  ...
  DstAlloc = dyn_cast<AllocaInst>(GetUnderlyingObject(DstOpnd, *DL));
  SrcAlloc = dyn_cast<AllocaInst>(GetUnderlyingObject(SrcOpnd, *DL));
...
bool CopyElisionLegacyPass::collectInstructionsToErase() {
...
template <typename StartPredicateT, typename InBetweenPredicateT,
          typename EndPredicateT>
bool CopyElisionLegacyPass::collectInBetweenInsToErase(
  ...
  // be possible if copy ctor is invoke and wan't not inlined)
  ...
  // Traverse through sub-graph that contains from all nodes
...
bool CopyElisionLegacyPass::isCopyOrInCopy(const Value &V) const {
...
bool CopyElisionLegacyPass::analyzeSrcAndDstUsers() {
...
bool CopyElisionLegacyPass::areAllNonTrivialUsersAfterCopy(
    const Instruction &I) {
...
bool CopyElisionLegacyPass::areAllNonTrivialUsersBeforeCopy(
    const Instruction &I) {
...
CopyElisionLegacyPass::analyzeSrcAndDstLifetimes() {
...
void CopyElisionLegacyPass::replaceDstObjWithSrcObj() {
  ...
  for (auto *U : DstObj->users()) {
    ...
    if (auto *UI = dyn_cast<Instruction>(U)) {
      if (!isa<AllocaInst>(SrcObj) && !DT->dominates(SrcObj, UI) &&
        ...
        assert(!isPotentiallyReachable(SrcObj, UI, nullptr, DT));
